"""Huấn luyện, validation và test Elman RNN phân loại DOA."""
...
from rnn_cau_hinh import cau_hinh_rnn as cfg
from rnn_cau_hinh.mo_hinh_rnn import ElmanRNNDOAClassifier
...
def get_cuda_device() -> torch.device:
    """Lấy CUDA device bắt buộc cho baseline RNN có AMP."""
    if not torch.cuda.is_available():
        raise RuntimeError(
            "Baseline RNN yêu cầu CUDA, nhưng torch.cuda.is_available() "
            "đang trả về False."
        )
...
def count_model_parameters(model: nn.Module) -> dict[str, int]:
    """Đếm parameter và xác minh model khớp cấu hình đối chứng."""
    ...
    counts = {
        ...
        "pre_layer_norms": sum(p.numel() for p in model.pre_norms.parameters()),
    ...
    if counts["total"] != cfg.EXPECTED_RNN_PARAMS:
        raise RuntimeError(
            f"RNN có {counts['total']:,} parameters, không khớp "
            f"EXPECTED_RNN_PARAMS={cfg.EXPECTED_RNN_PARAMS:,}."
        )
    if counts["trainable"] != cfg.EXPECTED_RNN_PARAMS:
        raise RuntimeError(
            f"RNN có {counts['trainable']:,} trainable parameters, không khớp "
            f"EXPECTED_RNN_PARAMS={cfg.EXPECTED_RNN_PARAMS:,}."
        )
...
def print_parameter_counts(counts: dict[str, int]) -> None:
    """In breakdown và so sánh số parameter ra Terminal."""
    print("\n===== MODEL PARAMETERS =====")
    print("Definition: 1 parameter = 1 scalar value stored in nn.Parameter.\n")
    print(f"RNN layers:           {counts['rnn_layers']:,}")
    ...
    print(f"Mamba-1 reference:    {cfg.REFERENCE_MAMBA1_PARAMS:,}")
    print(f"Difference:           {cfg.EXPECTED_PARAMETER_DIFFERENCE:+,}")
    print(
        "Difference percent:   "
        f"{cfg.EXPECTED_PARAMETER_DIFFERENCE_PERCENT:+.2f}%"
    )
    print("============================\n")
...
def write_test_summary(
    ...
    """Ghi parameter breakdown, so sánh và kết quả test tổng."""
    content = f"""\
...
RNN layers: {parameter_counts['rnn_layers']:,}
...
Mamba-1 reference parameters: {cfg.REFERENCE_MAMBA1_PARAMS:,}
RNN parameters: {parameter_counts['total']:,}
Difference: {cfg.EXPECTED_PARAMETER_DIFFERENCE:+,}
Difference percent: {cfg.EXPECTED_PARAMETER_DIFFERENCE_PERCENT:+.2f} %
...
def main() -> None:
    ...
    model = ElmanRNNDOAClassifier().to(device)
